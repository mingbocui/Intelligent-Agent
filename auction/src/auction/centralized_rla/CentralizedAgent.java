...
//    private List<Vehicle> vehicles;
    ...
    public SolutionSpace solution(List<Vehicle> vehicles, List<Task> tasks, long timeout) {
        ...
        while (true) {
            ...
            if (stuck || rnd.nextDouble() < this.randomSolutionSelection) {
                if (!currentMinSolutions.isEmpty() && rnd.nextDouble() < 0.5) {
                    if (this.log)
                        Utils.log(token, "\tselecting sol from queue, " + (currentMinSolutions.size() - 1) + " left to explore");
            ...
            if (false && nIterations % 1000 == 0)
                Utils.log(token, String.format("\tcurrent combinedCost (cost): %s (%s), lowest so far: %s",
        ...
        if (this.log)
            Utils.log(token, String.format("*** found solution after %d iterations, combinedCost (cost): %s (%s), profit: %s",
    ...
    private boolean outOfTime(long startTime, int nIterations, long timeout) {
        ...
        if (out)
            Utils.log(token, "we used " + executedTime + " of " + timeout + ", we had " + (timeout - executedTime) + " left");
        return out;
    }
}
