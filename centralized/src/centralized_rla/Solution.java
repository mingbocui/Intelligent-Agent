...
public class Solution {
    ...
    public Solution(List<Vehicle> allVehicles) {
        ...
        allVehicles.stream().forEach(vehicle -> {
            vehicleMapToActions.put(vehicle, new ArrayList<AgentAction>());
        });
    }
    ...
    public Vehicle closestVehicle(City taskCity, List<Vehicle> vehicles) {
        return null;
    }
    ...
    public Plan singlePlan(Vehicle vehicle) {
        ...
        for (AgentAction agentAction : vehicleMapToActions.get(vehicle)) {
            ...
            path.stream().forEach(city -> {
                vehiclePlan.appendMove(city);
            });
    ...
    public List<Plan> construcPlan() {
        ...
        this.allVehicles.stream().forEach(vehicle -> {
            plans.add(this.singlePlan(vehicle));
        });
